...
class FeaturizeLigandAtom(object):
    ...
    def __call__(self, data: ProteinLigandData):
        ...
        if 'pred_ligand_pos' in data and 'pred_ligand_v' in data:
            data.targetdiff_ligand_pos = torch.tensor(data.pred_ligand_pos)
            data.targetdiff_ligand_v = torch.tensor(data.pred_ligand_v)
        return data
...
class FeaturizeLigandBond(object):
    ...
    @property
    def feature_dim(self):
        return len(utils_data.BOND_TYPES)
    def __call__(self, data: ProteinLigandData):
...
class bond_angle_index(object):
    
    def __init__(self):
        super().__init__()

    def __call__(self, data: ProteinLigandData):
        data.bond_angle_index = self.get_bond_angle_atoms_positions(data.ligand_bond_index.detach())
        
        return data
    
  
    def get_bond_angle_atoms_positions(self, ligand_bond_index):
        ligand_bond_index = ligand_bond_index.numpy()
        row1, row2, row3 = [], [], []
        num_bonds = ligand_bond_index.shape[1]

        for i in range(num_bonds):
            atom_i = ligand_bond_index[0, i]
            atom_j = ligand_bond_index[1, i]

            for k in range(i+1, num_bonds):  # Start from i+1 to prevent double counting
                if (ligand_bond_index[0, k] == atom_i and ligand_bond_index[1, k] != atom_j) :
                    atom_k = ligand_bond_index[1, k] 
                    row1.append(atom_k)
                    row2.append(atom_i)
                    row3.append(atom_j)
        atom_index = torch.tensor([row1, row2, row3], dtype=torch.long)
        return atom_index
