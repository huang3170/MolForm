...
def parse_qvina_outputs(docked_sdf_path):
    print("Parsing file:", docked_sdf_path)
...
class QVinaDockingTask(BaseDockingTask):
    ...
    def __init__(self, pdb_block, ligand_rdmol, conda_env='MolDiff', tmp_dir='./tmp', use_uff=True, center=None,
                 size_factor=1.):
    ...
    def run(self, exhaustiveness=16):
        print(self.tmp_dir)
        print(self.receptor_id)
        commands = """
eval "$(conda shell.bash hook)"
conda activate {env}
cd {tmp}
# Prepare receptor (PDB->PDBQT)
/home/dhzhang/targetdiff/AutoDockTools_py3/AutoDockTools/Utilities24/prepare_receptor4.py -r {receptor_id}.pdb
# Prepare ligand
